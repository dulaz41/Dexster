...
mod dexster {
    ...
    impl Dexster {
        /// Constructs a new Dexster instance
        ...
        pub fn getPoolDetails(&self) -> (Balance, Balance, Balance, Balance) {
            (self.totalToken1, self.totalToken2, self.totalShares, self.fees)
        }
        ...
        pub fn provide(
            ...
            let new_share: u128 = self.shares
                .get(caller)
                .map(|val| val + share)
                .unwrap_or(share);
        ...
        pub fn withdraw(&mut self, _share: Balance) -> Result<(Balance, Balance), Error> {
            ...
            let newToken1Balance = self.token1Balance
                .get(caller)
                .map(|val| val + amountToken1)
                .unwrap();
            self.token1Balance.insert(caller, &newToken1Balance);
            let newToken2Balance = self.token2Balance
                .get(caller)
    ...
    mod tests {
        ...
        #[ink::test]
        fn faucet_works() {
            let mut contract = Dexster::new(0);
        ...
        fn trading_fees_works() {
            let mut contract = Dexster::new(100);
            contract.faucet(100, 200);
