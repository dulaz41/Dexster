...
mod erc20 {
    ...
    mod tests {
        ...
        fn assert_transfer_event(
            ...
            let decoded_event = <Event as scale::Decode>
                ::decode(&mut &event.data[..])
            ...
            for (n, (actual_topic, expected_topic)) in topics
                .iter()
                .zip(expected_topics)
                .enumerate() {
        ...
        fn allowance_must_not_change_on_failed_transfer() {
            ...
            assert_eq!(emitted_events_before, ink::env::test::recorded_events().count())
        }
        ...
        impl<X> scale::Encode for PrefixedValue<'_, '_, X> where X: scale::Encode {
            #[inline]
        ...
        fn encoded_into_hash<T>(entity: &T) -> Hash where T: scale::Encode {
            use ink::{ env::hash::{ Blake2x256, CryptoHash, HashOutput }, primitives::Clear };
    ...
    mod e2e_tests {
        ...
        async fn e2e_transfer(mut client: ink_e2e::Client<C, E>) -> E2EResult<()> {
            ...
            let contract_acc_id = client
                .instantiate("erc20", &ink_e2e::alice(), constructor, 0, None).await
                .expect("instantiate failed").account_id;

            // when
            let total_supply_msg = build_message::<Erc20Ref>(contract_acc_id.clone()).call(|erc20|
                erc20.total_supply()
            );
            let total_supply_res = client
                .call(&ink_e2e::bob(), total_supply_msg, 0, None).await
                .expect("total_supply failed");

            let bob_account = ink_e2e::account_id(ink_e2e::AccountKeyring::Bob);
            let transfer_to_bob = 500_000_000u128;
            let transfer = build_message::<Erc20Ref>(contract_acc_id.clone()).call(|erc20|
                erc20.transfer(bob_account.clone(), transfer_to_bob)
            );
            let _transfer_res = client
                .call(&ink_e2e::alice(), transfer, 0, None).await
                .expect("transfer failed");

            let balance_of = build_message::<Erc20Ref>(contract_acc_id.clone()).call(|erc20|
                erc20.balance_of(bob_account)
            );
            let balance_of_res = client
                .call(&ink_e2e::alice(), balance_of, 0, None).await
                .expect("balance_of failed");

            // then
            assert_eq!(total_supply, total_supply_res.return_value().unwrap(), "total_supply");
            assert_eq!(transfer_to_bob, balance_of_res.return_value().unwrap(), "balance_of");
        ...
        async fn e2e_allowances(mut client: ink_e2e::Client<C, E>) -> E2EResult<()> {
            ...
            let constructor = Erc20Ref::new(total_supply);
            let contract_acc_id = client
                .instantiate("erc20", &ink_e2e::bob(), constructor, 0, None).await
                .expect("instantiate failed").account_id;
            ...
            let transfer_from_result = client.call(
                &ink_e2e::charlie(),
                transfer_from,
                0,
                None
            ...
            let approve_call = build_message::<Erc20Ref>(contract_acc_id.clone()).call(|erc20|
                erc20.approve(charlie_account.clone(), approved_value)
            );
            client.call(&ink_e2e::bob(), approve_call, 0, None).await.expect("approve failed");
